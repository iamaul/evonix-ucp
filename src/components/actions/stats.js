...
export const getCountServerUsers = () => async dispatch => {
    try {
        const res = await api.get('/api/v1/server/stats/users');
...
export const getCountServerVehicles = () => async dispatch => {
    try {
        const res = await api.get('/api/v1/server/stats/player_vehicles');
...
export const getCountServerProperties = () => async dispatch => {
    try {
        const res = await api.get('/api/v1/server/stats/player_properties');
...
export const getCountServerUserApps = () => async dispatch => {
    try {
        const res = await api.get('/api/v1/server/stats/user_applications');
